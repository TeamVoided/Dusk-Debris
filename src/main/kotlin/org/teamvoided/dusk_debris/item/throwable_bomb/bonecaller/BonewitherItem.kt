...
open class BonewitherItem(block: Block, settings: Properties) : AbstractThrowableBombItem(block, settings) {

    override fun throwBomb(world: Level, user: Player, itemStack: ItemStack) {
        if (!world.isClientSide) {
            val bombItem = BonewitherEntity(user, world)
    ...
    override fun asProjectile(world: Level, pos: Position, stack: ItemStack, direction: Direction): Projectile {
        val bombEntity = BonewitherEntity(pos.x(), pos.y(), pos.z(), world)
